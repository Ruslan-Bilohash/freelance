...
function fl_langs(): array { global $FL_LANGS; return $FL_LANGS; }

function fl_detect_lang(): string
{
    global $base_path, $FL_LANGS;
    $codes = array_keys($FL_LANGS);

    if (!empty($_GET['lang']) && in_array($_GET['lang'], $codes, true)) {
        $chosen = $_GET['lang'];
        setcookie(FL_LANG_COOKIE, $chosen, [
    ...
    if (!empty($_COOKIE[FL_LANG_COOKIE]) && in_array($_COOKIE[FL_LANG_COOKIE], $codes, true)) {
        return $_COOKIE[FL_LANG_COOKIE];
    }
    return 'no';
}

$lang      = fl_detect_lang();
$lang_meta = $FL_LANGS[$lang] ?? $FL_LANGS['no'];
